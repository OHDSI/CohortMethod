...
{@use_covariate_drug_exposure} ? {
  ...
  {@use_covariate_drug_exposure_365d} ? {
	...
	AND de1.drug_concept_id not in (SELECT descendant_concept_id FROM concept_ancestor WHERE ancestor_concept_id IN (@target_drug_concept_id, @comparator_drug_concept_id))
