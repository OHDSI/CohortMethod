...
-- SQLite-friendly version (keeping BIGINT column types)

-- 1. Add 'gradient' to cm_likelihood_profile
CREATE TABLE @database_schema.@table_prefixcm_likelihood_profile_new AS
SELECT *,
       NULL AS gradient
...
-- 2. Add 'sdm_family_wise_min_p' and 'shared_sdm_family_wise_min_p' to cm_diagnostics_summary
CREATE TABLE @database_schema.@table_prefixcm_diagnostics_summary_new AS
SELECT *,
...
SELECT ((CAST(target_id AS BIGINT) * 2654435769 / 4096) & 1048575) * 4194304 +
       ((CAST(comparator_id AS BIGINT) * 2654435769 / 1024) & 4194303) AS target_comparator_id,
       target_id,
       comparator_id
FROM @database_schema.@table_prefixcm_result
GROUP BY target_id, comparator_id;
...
SELECT sequence_number,
       ...
       subjects,
       exposure_id,
       ((CAST(target_id AS BIGINT) * 2654435769 / 4096) & 1048575) * 4194304 +
       ((CAST(comparator_id AS BIGINT) * 2654435769 / 1024) & 4194303) AS target_comparator_id,
       analysis_id,
       outcome_id,
       database_id
