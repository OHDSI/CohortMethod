...
simulateCohortData <- function(cohortDataSimulationProfile, n=10000){
  ...
  covariatePrevalence <- cohortDataSimulationProfile$covariatePrevalence[names(cohortDataSimulationProfile$covariatePrevalence) != "1"]

  personsPerCov <- rpois(n = length(covariatePrevalence),
                         lambda = covariatePrevalence*n)
