...
#' Assessing Covariate Balance with Small Sample Sizes. Stat Med. 2025 Aug;44(18-19):e70212.
#'
#' @export
...
doMetaAnalysisSingleCovariate <- function(group) {
  group <- group |>
    filter(!is.na(.data$afterMatchingSdmVariance))
  if (nrow(group) == 0) {
    return(NULL)
  }
...
#' Perform balance meta-analysis
#'
#' @param balances    A list of balance data frames as generated by [computeCovariateBalance()].
...
#'
#' @details
#' Perform a random-effects meta-analysis on the covariate balance standardized difference of means.
#' This is recommended when using balance hypothesis testing, as any single database may be
#' underpowered to detect imbalances that are systemic across the databases. (Hripcsak et al. 2025)
#'
#' @references
#' Hripcsak G, Zhang L, Chen Y, Li K, Suchard MA, Ryan PB, Schuemie MJ (2025)
#' Assessing Covariate Balance with Small Sample Sizes. Stat Med. 2025 Aug;44(18-19):e70212.
#'
#' @returns
#' A new balance data frame.
#'
#' @export
computeBalanceMetaAnalysis <- function(balances, threshold = 0.1, alpha = 0.05, maxCores = 1) {
  errorMessages <- checkmate::makeAssertCollection()
  checkmate::assertList(balances, min.len = 2, add = errorMessages)
  for (balance in balances) {
    checkmate::assertDataFrame(balance, add = errorMessages)
    checkmate::assertNames(colnames(balance), must.include = c("covariateId",
                                                               "covariateName",
                                                               "beforeMatchingStdDiff",
                                                               "beforeMatchingSdmVariance",
                                                               "afterMatchingStdDiff",
                                                               "afterMatchingSdmVariance"), add = errorMessages)
  }
  checkmate::assertNumber(threshold, lower = 0, add = errorMessages)
  checkmate::assertNumber(alpha, lower = 0, null.ok = TRUE, add = errorMessages)
  checkmate::reportAssertions(collection = errorMessages)
  if (isFALSE(tryCatch(find.package("meta"), error = function(e) FALSE))) {
    warning("The 'meta' package must be installed to use this function")
    return()
  }
  ...
  groups <- bind_rows(balances) |>
    group_by(.data$covariateId) |>
    group_split()
  balance <- ParallelLogger::clusterApply(cluster, groups, doMetaAnalysisSingleCovariate)
  balance <- bind_rows(balance)
  balance$beforeMatchingSdmP <- computeBalanceP(balance$beforeMatchingStdDiff, balance$beforeMatchingSdmVariance, threshold)
  balance$beforeMatchingBalanced <-  balance$beforeMatchingSdmP > alpha / nrow(balance)

  balance$afterMatchingSdmP <- computeBalanceP(balance$afterMatchingStdDiff, balance$afterMatchingSdmVariance, threshold)
  balance$afterMatchingBalanced <-  balance$afterMatchingSdmP > alpha / nrow(balance)
