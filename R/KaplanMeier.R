...
#' as described in Galimberti eta al (2002).
#'
...
#' @param nBootstrap            Number of bootstrap samples for the adjusted KM for stratified data.
...
plotKaplanMeier <- function(population,
  ...
  if (is.null(population$stratumId) || length(unique(population$stratumId)) == nrow(population)/2) {
    ...
    survTarget <- adjustedKm(stratumId = population$stratumId[idx],
                             time = population$survivalTime[idx],
                             y = population$y[idx],
                             nBootstrap = nBootstrap)
    survTarget$strata <- treatmentLabel
    idx <- population$treatment == 0
    survComparator <- adjustedKm(stratumId = population$stratumId[idx],
                                 time = population$survivalTime[idx],
                                 y = population$y[idx],
                                 nBootstrap = nBootstrap)
