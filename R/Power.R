...
#'
#' @details
#' Compute the minimum detectable relative risk (MDRR) for a given study population, using the actual observed sample size and number
#' of outcomes. Currently, only computations for Cox models are implemented. For Cox model, the computations by Schoenfeld (1983)
#' is used.
#'
#' @param population       A data frame describing the study population as created using the \code{\link{createStudyPopulation}} function.
#'                              This should at least have these columns: subjectId, treatment, outcomeCount, timeAtRisk.
...
computeMdrr <- function(population, alpha = 0.05, power = 0.8, twoSided = TRUE, modelType = "cox") {
  ...
  # denom <- sqrt(pA) * sqrt(pB) * sqrt(totalEvents)
  # mdrr <- exp(zBeta / denom + z1MinAlpha / denom)
...
#'
#' @details
#' Get the distribution of follow-up time as quantiles. Follow-up time is defined as time-at-risk, so not censored at the
#' outcome.
#'
#' @param population       A data frame describing the study population as created using the \code{\link{createStudyPopulation}} function.
#'                              This should at least have these columns: treatment, timeAtRisk.
...
#'
#' @details
#' Plot the distribution of follow-up time, stratified by treatment group.Follow-up time is defined as time-at-risk, so not censored at the
#' outcome.
#'
#' @param population       A data frame describing the study population as created using the \code{\link{createStudyPopulation}} function.
#'                              This should at least have these columns: treatment, timeAtRisk.
...
plotFollowUpDistribution <- function(population,
  ...
  plot <- ggplot2::ggplot(d, ggplot2::aes(x = followUp, y = sumCount, group = label, color = label)) +
