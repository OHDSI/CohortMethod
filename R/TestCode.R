...
testCode <- function(){
  ...
  dbSendUpdate(conn,"CREATE TEMP TABLE cyclops_outcome_input_for_ps AS SELECT * FROM temp_outcomes")
  dbSendUpdate(conn,"CREATE TEMP TABLE cyclops_covariate_input_for_ps AS SELECT * FROM temp_covariates")
  ...
  cyclopsData1 <- createCyclopsData.ffdf(outcomes1,covariates1,modelType="lr")
  
  outcomes2 <- dbGetQuery(conn,outcomeSql)
  covariates2 <- dbGetQuery(conn,covariateSql)
  cyclopsData2 <- createCyclopsData(outcomes2,covariates2,modelType="lr")
  
  cyclopsData3 <- dbCreateCyclopsData(conn,outcomeSql,covariateSql,modelType = "lr")
  
  cyclopsFit1 <- fitCyclopsModel(cyclopsData1, prior = prior("laplace",0.1))
  cyclopsFit2 <- fitCyclopsModel(cyclopsData2, prior = prior("laplace",0.1))
  cyclopsFit3 <- fitCyclopsModel(cyclopsData3, prior = prior("laplace",0.1))
  min(coef(cyclopsFit1) == coef(cyclopsFit2))
  min(coef(cyclopsFit2) == coef(cyclopsFit3))
  min(coef(cyclopsFit1) == coef(cyclopsFit3))
  ...
  dbSendUpdate(conn,"CREATE TABLE temp_outcomes AS SELECT * FROM cyclops_outcome_input_for_ps")
  dbSendUpdate(conn,"CREATE TABLE temp_covariates AS SELECT * FROM cyclops_covariate_input_for_ps")
  ...
  startQuery <- Sys.time()
  cyclopsData <- dbCreateCyclopsData(conn,outcomeSql,covariateSql,modelType = "lr")
  cyclopsDelta <- Sys.time() - startQuery
  writeLines(paste("Load took", signif(cyclopsDelta,3), attr(cyclopsDelta,"units")))
  ...
  cyclopsData <- createCyclopsData.ffdf(outcomes,covariates,modelType="lr")
  cyclopsDelta <- Sys.time() - startQuery
  writeLines(paste("Load took", signif(cyclopsDelta,3), attr(cyclopsDelta,"units")))
  ...
  cyclopsFit <- fitCyclopsModel(cyclopsData, prior = prior("laplace",0.1))
  p <- predict(cyclopsFit)
  if (is.null(outcomes)){
    ySql <-"SELECT row_id,y FROM #cyclops_outcome_input_for_ps"
  ...
  dbSendUpdate(conn,"CREATE TABLE temp_outcomes_o AS SELECT * FROM cyclops_outcome_input_for_outcome")
  dbSendUpdate(conn,"CREATE TABLE temp_covariates_o AS SELECT * FROM cyclops_covariate_input_for_outcome")
  
  # Option 2: Retrieve stored tables:
  dbSendUpdate(conn,"CREATE TEMP TABLE cyclops_outcome_input_for_outcome AS SELECT * FROM temp_outcomes_o")
  dbSendUpdate(conn,"CREATE TEMP TABLE cyclops_covariate_input_for_outcome AS SELECT * FROM temp_covariates_o")
  
  ### Load outcome data, and merge with stratification from matching ***
  outcomeSql <-"SELECT * FROM #cyclops_outcome_input_for_outcome ORDER BY row_id"
  outcomeSql <- translateSql(outcomeSql,"sql server",connectionDetails$dbms)$sql
  covariateSql <-"SELECT * FROM #cyclops_covariate_input_for_outcome ORDER BY row_id, covariate_id"
  covariateSql <- translateSql(covariateSql,"sql server",connectionDetails$dbms)$sql
  ...
  #cyclopsData <- createCyclopsData(outc,cova,modelType="clr", addIntercept=FALSE)
