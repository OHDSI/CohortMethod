...
drawAttritionDiagram <- function(outcomeModel,
  ...
  if ("notPriorCount" %in% names(counts)){
    data <- addStep("Does not have the outcome\nprior to index date", counts$notPriorCount, data)
  }
  if ("matchedTrimmedCount" %in% names(counts)){
    data <- addStep("Kept during matching and/or\ntrimming on propensity score", counts$matchedTrimmedCount, data)
  }
  data$leftBoxText[length(data$leftBoxText) + 1] <- paste("Study population:\n",treatmentLabel,": n = ",data$currentCounts[2],"\n",comparatorLabel,": n = ",data$currentCounts[1],sep="")
  ...
  box <- function(p,x,y){
    p = p + ggplot2::geom_rect(ggplot2::aes_string(xmin=x-(boxWidth/2)+shadowOffset, ymin=y-(boxHeight/2)- shadowOffset, xmax=x+(boxWidth/2)+ shadowOffset, ymax=y+(boxHeight/2)- shadowOffset), fill = rgb(0,0,0,alpha=0.2))
    p = p + ggplot2::geom_rect(ggplot2::aes_string(xmin=x-(boxWidth/2), ymin=y-(boxHeight/2), xmax=x+(boxWidth/2), ymax=y+(boxHeight/2)), fill = rgb(0.94,0.94,0.94), color = "black")
    return(p)
  }
