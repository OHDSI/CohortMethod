...
analysisSum <- vignetteAnalysisSummary
...
1. **Hypothesis of interest**: arguments that are specific to a hypothesis of interest, in the case of the cohort method this is a combination of drug, comparator, and outcome (and indication if required).
...
We defined the target to be celecoxib (concept ID 1118084), the comparator to be diclofenac (concept ID 1124300), and the outcomes of interest are GI-bleed (concept ID 192671) and a large number of negative control outcomes. In this example, we also specify that people with prior exposure to any NSAID should be excluded when investigating these drugs and outcomes. We furthermore specify that NSAIDS cannot be in the list of covariates. We use some SQL against the vocabulary to find the concept IDs of all NSAIDS.

A convenient way to save `drugComparatorOutcomeList` to file is by using the `saveDrugComparatorOutcomesList` function, and we can load it again using the `loadDrugComparatorOutcomesList` function.
...
We can easily create more analyses, for example by using matching or stratification, or by using more sophisticated outcome models:
...
The result of the `runCmAnalyses()` is a data frame with one row per drug-target(-indication)-outcome-analysis combination. It provides the file names of the intermediate and end-result files that were constructed. For example, we can retrieve and plot the propensity scores for the combination of our drug, comparator, outcome of intererst, and last analysis:
...
if (file.exists("s:/temp/CohortMethodOutput/outcomeModelReference.rds")){
...
Note that some of the file names will appear several times in the table. For example, analysis 4 and 5 only differ in terms of the outcome model, and will share the same propensity score files.
...
This tells us, per drug-target(-indication)-outcome-analysis combination, the estimated relative risk and 95% confidence interval, as well as the number of people in the treated and comparator group (after trimming and matching if applicable), and the number of outcomes observed for those groups within the specified risk windows.
