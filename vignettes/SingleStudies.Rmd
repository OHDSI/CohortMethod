...
The last two lines define the `cdmSchema` and `resultSchema` variables, which we'll use later to tell R where the data in CDM format live, and where we want to write intermediate and result tables.
...
data.frame(cohort_concept_id = c(1,2,3),count=c(113492,342370,351289))
```
