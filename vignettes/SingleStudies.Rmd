...
if (file.exists("c:/temp/vignetteCohortData")){
  cohortData <- loadCohortMethodData("c:/temp/vignetteCohortData")
} 
```
```{r echo=FALSE,message=FALSE}
if (file.exists("c:/temp/vignetteCohortData")){
...
**Important**: The target and comparator drug must not be included in the covariates, including any descendant concepts. If the `targetDrugConceptId` and `comparatorDrugConceptId` arguments represent real concept IDs, you can set the `excludeDrugsFromCovariates` argument to TRUE and automatically the drugs
and their descendants will be excluded from the covariates. However, if the `targetDrugConceptId` and `comparatorDrugConceptId` arguments do not represent concept IDs, such as in the example above, you will need to manually add the drugs and descendants to the `excludedCovariateConceptIds` of the`covariateSettings` argument.
