...
This vignette describes how you can use the CohortMethod package to perform a single new-user cohort study. It starts by describing how you can define the exposure, comparator, and outcome cohorts, and details how the analysis is performed to produce a single estimate of the average treatment effect.

As an example, we will perform a study evaluating the effect of coxibs versus non-selective NSAIDs on hospitalization due to GI bleeding.
...
The CohortMethod package is maintained in a [Github repository](https://github.com/OHDSI/CohortMethod), and has dependencies on other packages in the repository. These can be downloaded and installed from within R using the `devtools` package:
...
The last two lines define the cdmSchema and resultSchema variables, which we'll be using later.
